...
const ALERTS_API = `${window.location.origin}/api/alerts`;
...
document.addEventListener('DOMContentLoaded', () => {
    setupChat();
    addWelcomeMessage();
    // Check for forecast-based alerts after the page loads
...
window.openWeatherChat = function () {
    toggleChat(true);
};
// ── Messages ───────────────────────────────────────────────────
...
async function checkWeatherAlerts() {
    try {
        ...
        const longitude = window.currentLon;
        const city = window.currentCity || null;
        window.checkWeatherAlerts = checkWeatherAlerts;

        // Do not call the API until coordinates are available
        if (latitude == null || longitude == null) {
            console.log('Location not available for alert checking.');
            return;
        }
        ...
        console.log("Alert API data:", data);
        if (data.has_alert && data.alerts?.length > 0) {
                ...
                const alertHtml = `
                    <div class="weather-alert ${alert.severity}">
                        ...
                        <small>
                            ...
                            Forecast-based alert — not an official IMD warning.
                        </small>
                    </div>
                `;
...
async function sendMessage(prefilledMessage = '') {
    const message =
        typeof prefilledMessage === 'string'
    ...
    try {
        console.log("Selected city:", window.currentCity);
