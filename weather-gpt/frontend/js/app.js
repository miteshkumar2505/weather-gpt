...
async function loadWeatherByCity(city) {
    ...
    try {
        ...
        currentLat = weather.lat;
        currentLon = weather.lon;
...
function renderMap(lat, lon, city) {
    ...
    // Fix map rendering issues inside dynamically shown containers
    setTimeout(() => {
    ...
    if (!alerts || alerts.length === 0) {
        container.innerHTML = `
            <div class="no-alerts">
                ...
                <div class="no-alerts-text">No active weather alerts for this area</div>
            </div>
        `;
